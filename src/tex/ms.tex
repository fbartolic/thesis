...
Scientific progress in 21st-century astronomy research often depends on the careful 
statistical analysis of noisy time series datasets. The process describing how these data 
were ``generated'' consists of the physical phenomenon of interest,
for instance, an exoplanet blocking or deflecting the light of a distant star,
the noise introduced by the measurement process, and the presence of the Earth's 
atmosphere. The task for the astronomer is to encode that process as best as possible 
in an approximate \emph{model} and then ``fit'' that model to data. 
The common theme of this thesis is building
\emph{computational and statistical infrastructure} to do statistical 
inference with complex astrophysical models. More specifically, 
I worked on problems in two 
particular domains within astronomy: gravitational microlensing and occultation/eclipse
mapping. Both of these fields are concerned with 
measuring the brightness of distant stars as a function of time to infer
properties of exoplanets, stars and dark compact objects such 
as black holes and neutron stars.
I used recent advancements in statistics, machine learning and computer science 
to construct a new lens through which to view some well-known problems in 
these fields. In the context of microlensing, I have developed a new open-source package 
\ssf{caustics}\footnote{\url{https://github.com/fbartolic/caustics}}
for computing the magnification in single, binary, and triple-lens microlensing events
with some notable improvements over past methods. I then tackled some foundational questions
on the statistical analysis of single-lens and multiple-lens microlensing events.
Using a case study format, I have developed a new paradigm for modelling degenerate 
single-lens microlensing events such as annual parallax events. 
I have also shown that methods for analyzing multiple-lens microlensing events
commonly used in the literature are deeply flawed because of the non-smooth nature of the 
problem. 
Besides microlensing, I built models for reconstructing two-dimensional 
emission maps of spherical bodies (residing within and outside of the Solar System)
from one-dimensional photometric occultation light curves.
Together with collaborators, I have developed a novel method for reconstructing spatial maps of volcanic emission on Jupiter's moon Io from 
occultation light curves.
I then used the same method in a much more challenging regime of exoplanet eclipse 
mapping in ordered to
to explore the possibility of detecting weather and climate change on Hot Jupiters 
using simulated photometric JWST eclipse 
light curves. I found that, unfortunately, there is very little hope of inferring anything but 
the crudest maps of the planet's dayside emission.
...
statistics. Fig~\ref{fig:ads_keyword_bayesian} shows that the number of entries in
the NASA/ADS database containing the keyword ``Bayesian'' each year is growing at an 
accelerating rate. 
Most of this thesis is devoted to the application of Bayesian methods to specific 
astrophysical problems.
Although these methods have been around for some time, they have only become 
popular  recently because they have been too computationally expensive.
...
\begin{figure}
    \begin{centering}
        ...
        \script{chapter_1/ads_keyword_bayesian.py}
    \end{centering}
\end{figure}

Having situated the work presented in this thesis in the present moment and pointed
out relevant scientific and technological in recent decades, I now focus on astronomy 
...
we now know \citep{2012Natur.481..167C} that there is, on average, one planet per
...
The focus of this thesis is mostly on building methods which will enable the scientific 
discovery of exoplanet and planetary science rather than on the discovery itself. 
Wherever possible, I approach these problems using first principles thinking 
...
a \emph{gravitational lens}. He warned that it wouldn't be a lens in the usual sense
because it does not have a focal length.
...
of the Sun\footnote{He concluded that the observed deflection was in agreement
    with GR. However, it is doubtful that the data was actually good enough to
...
time. Because of this, microlensing observations focus on the densest region of
...
do not detect any light from the lens itself. The collected photons originate from a
...
essential to have high-cadence observations of the source star. The way surveys campaigns
...
questions about \emph{populations} of objects with these new, but also existing datasets,
...
The assumptions that existing methods for modelling microlensing events rely on
...
surfaces. The key idea is as follows. Although at any given time we only observe the
...
to capture longitudinal variations in intensity. Real exoplanet atmospheres 
...
scales, the deflection  is impossible to detect in a single background source, 
but one can tease out the effect in a statistical sense.
Strong lensing refers to the lensing of individual objects. It is further divided into
...
\begin{itemize}
    ...
    \item Massless particles such as photons follow the null geodesics -- minimum
          distance paths in curved spacetime, obtained by solving the
...
\begin{equation}
    ...
    \hquad,
\end{equation}
where $x$ parametrises the straight line distance between the observer and the lens and
...
Equation~\ref{eq:deflection_angle_general} is only valid when the linearised
...
the location of the source, observer, and lens can be parametrised using
angular coordinates (or Cartesian coordinates if the distance to the lens and
...
Because gravitational lensing does not involve any additional emission or
absorption along a deflected ray of light and the wavelength of the light
...
Assuming that the source has infinitesimal size and it is located at angular 
 an infinitesimal source size at an angular location
...
observer, the lens, and the source is rectilinear (acceleration can be neglected).
...
can only measure the flux of the source star, which is usually contaminated by 
...
$F_\mathrm{base}$ which is a direct observable, but it does not require an extra evaluation 
...
this case is not so important because in the we will always marginalise (integrate out)
...
the \emph{position} of the source star when it is in the vicinity of the lens.
For a point source and a single-lens, we can
obtain the centroid of the two images by weighting their positions
...
\begin{itemize}
    ...
    \item Decreasing the source radius $\rho_\star$ results in narrower arcs of the
          images with smaller radial extent.
\end{itemize}
...
The model parameters which determine the magnification $A(t)$ are 
...
There exist another parametrisation of the trajectory in which we fit for the local
...
$\mathbf{y}$ is a quantity of central importance because it defines 
the body's \emph{map} (the specific intensity at every point).
\citet{2019AJ....157...64L} show that we can represent $y$ in a polynomial basis (see
...
independent.
The parameters $l$ set the length scale which determines how quickly the covariance decays
...
Because there are so many applications of of autodiff,
...
\begin{figure}[t]
    \begin{centering}
        ...
        \caption{
            Visualization of the tests which determine whether the 
...
three angles:  the counterclockwise obliquity angle \textsf{obl} between the $\hat{y}$
 ...
 phase angle \textsf{theta} which determines the rotation of the sphere around the 
...
We showed that the key factor which determines whether a given spatial feature can be 
...
observations is from the 90s, the second is from the 2010s. 
...
which determine how the basis maps add together to form the emission map at any given time.
