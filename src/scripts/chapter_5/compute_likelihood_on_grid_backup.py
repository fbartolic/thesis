...
def ll_fn_vbb(x, pointwise=False):
...
def model(t, t0_est, fobs, ferr):
    ln_t0 = numpyro.sample("ln_t0", dist.Normal(jnp.log(t0_est), 20.))
    ...
    ln_u0 = numpyro.sample("ln_u0", dist.Normal(jnp.log(1e-03), jnp.log(2)))
    ln_tE = numpyro.sample("ln_tE", dist.Normal(2., 6.))
    alpha = numpyro.sample("alpha", dist.Uniform(-np.pi, np.pi))
    ln_rho = numpyro.sample("ln_rho", dist.Normal(jnp.log(5e-04), jnp.log(0.5)))
...
params_init = {
    'ln_s':jnp.log(2.218), 
    'ln_q':jnp.log(0.3392), 
    'ln_t0':jnp.log(7847.5), 
    'ln_u0':jnp.log(0.375), 
    'ln_tE':jnp.log(241.), 
    'alpha': 4.56 - np.pi ,
    'ln_rho':jnp.log(0.00097),
}
t0_est = jnp.exp(params_init['ln_t0'])


log_likelihood_fn = lambda params: numpyro.infer.util.log_likelihood(
    model,
    params, 
...
def compute_loglike_slice(
    ...
    params = {
        'ln_s':ln_s if ln_s is not None else params_init['ln_s']*jnp.ones(npts),
        'ln_q':ln_q if ln_q is not None else params_init['ln_q']*jnp.ones(npts),
        'ln_t0':ln_t0 if ln_t0 is not None else params_init['ln_t0']*jnp.ones(npts),
        'ln_u0': ln_u0 if ln_u0 is not None else params_init['ln_u0']*jnp.ones(npts),
        'ln_tE': ln_tE if ln_tE is not None else params_init['ln_tE']*jnp.ones(npts),
        'alpha': alpha if alpha is not None else params_init['alpha']*jnp.ones(npts),
        'ln_rho': ln_rho if ln_rho is not None else params_init['ln_rho']*jnp.ones(npts),
    }
    return lax.map(log_likelihood_fn, params)
...
# PLOT 1: s vs u0
ngrid = 200
_s_linspace = jnp.linspace(0.1, 3., ngrid)
_u0_linspace = jnp.linspace(0.01, 0.7, ngrid)
s_grid, u0_grid = jnp.meshgrid(_s_linspace, _u0_linspace)

ll_grid_s_u0 = compute_loglike_slice(
    len(s_grid.reshape(-1)), 
    ln_s=jnp.log(s_grid.reshape(-1)), 
    ln_u0=jnp.log(u0_grid.reshape(-1)),
)

# zoom in
_s_linspace_zoom = jnp.linspace(1.9, 2.4, ngrid)
_u0_linspace_zoom = jnp.linspace(0.28, 0.45, ngrid)
s_grid_zoom, u0_grid_zoom = jnp.meshgrid(_s_linspace_zoom, _u0_linspace_zoom)

ll_grid_s_u0_zoom = compute_loglike_slice(
    len(s_grid_zoom.reshape(-1)), 
    ln_s=jnp.log(s_grid_zoom.reshape(-1)), 
    ln_u0=jnp.log(u0_grid_zoom.reshape(-1)),
)

# Plot 2: alpha vs t0 
_alpha_linspace = jnp.linspace(-np.pi, np.pi, ngrid)
_t0_linspace = jnp.linspace(7820, 7860, ngrid)
alpha_grid, t0_grid = jnp.meshgrid(_alpha_linspace, _t0_linspace)

ll_grid_alpha_t0 = compute_loglike_slice(
    len(alpha_grid.reshape(-1)), 
    ln_t0=jnp.log(t0_grid.reshape(-1)), 
    alpha=alpha_grid.reshape(-1),
)

# zoom in
_alpha_linspace_zoom = jnp.linspace(0.9, 1.8, ngrid)
_t0_linspace_zoom = jnp.linspace(7845, 7855, ngrid)
alpha_grid_zoom, t0_grid_zoom = jnp.meshgrid(_alpha_linspace_zoom, _t0_linspace_zoom)

ll_grid_alpha_t0_zoom = compute_loglike_slice(
    len(alpha_grid_zoom.reshape(-1)),
    ln_t0=jnp.log(t0_grid_zoom.reshape(-1)),
    alpha=alpha_grid_zoom.reshape(-1),
)

# Plot 3: q vs tE
_q_linspace = jnp.linspace(0.2, 0.5, ngrid)
_tE_linspace = jnp.linspace(200, 300, ngrid)
q_grid, tE_grid = jnp.meshgrid(_q_linspace, _tE_linspace)

ll_grid_q_tE = compute_loglike_slice(
    len(q_grid.reshape(-1)),
    ln_tE=jnp.log(tE_grid.reshape(-1)),
    ln_q=jnp.log(q_grid.reshape(-1)),
)

# zoom in
_q_linspace_zoom = jnp.linspace(0.3, 0.366, ngrid)
_tE_linspace_zoom = jnp.linspace(225, 250, ngrid)
q_grid_zoom, tE_grid_zoom = jnp.meshgrid(_q_linspace_zoom, _tE_linspace_zoom)

ll_grid_q_tE_zoom = compute_loglike_slice(
    len(q_grid_zoom.reshape(-1)),
    ln_tE=jnp.log(tE_grid_zoom.reshape(-1)),
    ln_q=jnp.log(q_grid_zoom.reshape(-1)),
)
