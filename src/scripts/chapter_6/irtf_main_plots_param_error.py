...
def make_plots(
    ...
    date_in = lc_in.time[0].datetime.strftime("%Y-%m-%d %H:%M")
    date_eg = lc_eg.time[0].datetime.strftime("%Y-%m-%d %H:%M")
    ...
    if gp:
        fig.savefig(
            f"irtf_{year}_param_error.pdf", bbox_inches="tight", dpi=500
        )
    else:
        fig.savefig(
            f"irtf_{year}_no_GP_param_error.pdf",
            bbox_inches="tight",
            dpi=500,
        )
...
with open("scripts/irtf_1998_samples_cubic.pkl", "rb") as handle:
...
with open("scripts/irtf_2017_samples_cubic.pkl", "rb") as handle:
    samples = pkl.load(handle)
...
make_plots(
    ...
    res_yticks,
    res_ylim,
    cmap_norm=colors.LogNorm(vmin=20, vmax=1400),
)
