...
from io_mapping_utils import get_median_map
...
def get_spot_position_and_power(samples, bounds=None, nsamples=200):
    """
    ...
    for n in np.random.randint(len(samples["x_in"]), size=nsamples):
        ...
        map[1:, :] = -map[1:, :]
        #        img = map.render(res=resol, projection="rect")
        #        lats, lons = map.get_latlon_grid(res=resol, projection="rect")
...
# Load Galileo map
combined_mosaic = Image.open(paths.data/"mapping_io/Io_color_merged_clon180.jpg")
...
map = starry.Map(20)
lat_grid, lon_grid = map.get_latlon_grid(res=resol, projection="rect")
...
def make_plot(
    ...
    # Estimate spot positions and emitted power
    (
        spot1_lat,
        spot1_lon,
        spot1_power_in,
    ...
    print_percentiles((360 - spot1_lon) % 360, "spot W longitude")
    ...
    print_percentiles((360 - spot2_lon) % 360, "spot W longitude")
    ...
    median_map_rect_in = get_median_map(
        20, samples["x_in"], projection="rect", nsamples=50, resol=resol
    )
    ...
    ax[0].imshow(shifted_map, extent=(-180, 180, -90, 90), zorder=-1)
    ax[0].set(
        xlim=xlim1,
        ylim=ylim1,
    )
    ...
    ax[1].imshow(shifted_map, extent=(-180, 180, -90, 90), zorder=-1)
    ax[1].set(
        xlim=xlim2,
        ylim=ylim2,
    )
    ...
    for a in ax:
        ...
        # Hack to change E longitude to W longitude, SS papers usually use
        # W longitude
        a.xaxis.set_major_formatter(
            ticker.FuncFormatter(lambda x, pos: ("%g") % ((360 - x) % 360))
        )
        a.set_xlabel("West longitude [deg]")
        a.set_rasterization_zorder(0)

    fig.savefig(paths.figures/f"irtf_{year}_spots.pdf", bbox_inches="tight", dpi=200)


# Plots for the the 1998 pair of light curves
with open(paths.data/"output/mapping_io/irtf_1998_samples.pkl", "rb") as handle:
...
with open(paths.data/"output/mapping_io/irtf_2017_samples.pkl", "rb") as handle:
