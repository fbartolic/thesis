...
def fit_model(ydeg_inf, lc_in, lc_eg):
    ...
    def model():
        ...
        # The Horseshoe prior on p
        p = numpyro.deterministic("p", tau0 * tau_raw * lamda_raw * beta_raw)
        x = jnp.dot(P2Y, p)
        ...
        sigma = numpyro.sample(
            "sigma_gp",
            dist.HalfNormal(0.5 * np.array([f_err_in[0], f_err_eg[0]])),
        )
        ...
        err_in_scale = numpyro.sample(
            "err_in_scale", dist.HalfNormal(f_err_in[0])
        )
        err_eg_scale = numpyro.sample(
            "err_eg_scale", dist.HalfNormal(f_err_eg[0])
        )
...
with open("regular_horseshoe/irtf_1998_samples.pkl", "wb") as handle:
...
with open("regular_horseshoe/irtf_2017_samples.pkl", "wb") as handle:
