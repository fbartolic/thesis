...
\begin{equation}
    L L(\boldsymbol{\mu}, \boldsymbol{\Sigma})=\log p(\mathcal{D} \lvert \boldsymbol{\mu}, \boldsymbol{\Sigma})=\frac{N}{2} \log |\vec{\Sigma}^{-1}|-\frac{1}{2} \sum_{n=1}^{N}\left(\boldsymbol{y}_{n}-\boldsymbol{\mu}\right)^{\top} \vec{\Sigma}^{-1}\left(\boldsymbol{y}_{n}-\boldsymbol{\mu}\right)
\end{equation}
...
\begin{equation}
    \hat{\boldsymbol{\Sigma}}=\frac{1}{N} \sum_{n=1}^{N}\left(\boldsymbol{y}_{n}-\overline{\boldsymbol{y}}\right)\left(\boldsymbol{y}_{n}-\overline{\boldsymbol{y}}\right)^{\top}
\end{equation}
...
(\emph{independent variables}, \emph{covariates}, or \emph{features}) and a
vector of \emph{regression coefficients} or \emph{weights} $\vec{\theta}\in
    \mathbb{R}^p$. This is a very general model because the matrix $\vec{M}$ can
consist of basis functions such as polynomials, spherical harmonics, Fourier
modes, wavelets etc. so although the model is linear in the parameters
(regression coefficients), it is not generally linear in the raw input data. If
we assume that the observations $\vec{y}$ have Gaussian noise we can write down
the likelihood of this model as
\begin{equation}
    p(\vec{y}\lvert \vec{\theta})=\mathcal{N}(\vec{y}\lvert\vec{M}\vec{\theta}, \vec{C})
\end{equation}
...
\begin{equation}
    \operatorname{NLL}(\vec{\theta})=\frac{1}{2}(\vec{M} \vec{\theta}-\vec{y})^{\top}\vec{C}^{-1}(\vec{M} \vec{\theta}-\vec{y})
\end{equation}
...
In that case the MLE solution for the weights $\vec{\theta}$ is the solution to
the following (convex) optimization problem:
\begin{equation}
    \hat{\vec{\theta}}=
    \underset{\boldsymbol{\theta}}{\operatorname{argmin}}\|\vec{y}-\vec{M} \vec{\theta}\|_{2}^{2}
...
\begin{equation}
    \hat{\vec{\theta}}=(\vec{M}^{\top} \vec{M})^{-1} \vec{M}^{\top} \vec{y}
    \label{eq:ols_solution}
\end{equation}
under the assumption that $\vec{M}\vec{M}^\top$ is invertible which is usually the case.
...
to
\begin{equation}
    \hat{\vec{\theta}}=(\vec{M}^{\top}\vec{C}^{-1}\vec{M})^{-1} \vec{M}^{\top}\vec{C}^{-1} \vec{y}
...
\begin{equation}
    \hat{\vec{\theta}}=
    \underset{\boldsymbol{\theta}}{\operatorname{argmin}}\|\vec{y}-\vec{M} \vec{\theta}\|_{2}^{2} + \lambda \|\vec{\theta}\|_2^2
    \label{eq:ols_objective_l2}
\end{equation}
where $\lambda >0$ is a regularization parameter
which penalizes large magnitudes of the coefficients $\vec{\theta}$. This is also a convex
optimization problem and the solution is
\begin{equation}
    \hat{\vec{\theta}}=(\vec{M}^{\top} \vec{M} + \lambda\vec{I})^{-1} \vec{M}^{\top} \vec{y}
...
literature\footnote{One of the many things that make statistics confusing is that very
    similar methods often have special names which obscures the big picture. The vast majority
...
case of linear regression $(p > n)$ there are multiple choices for
$\vec{\theta}$ which perfectly fit the data. The solution in that case is
defined to be a minimum-norm parameter vector $\vec{\theta}$ that interpolates
the data ($\vec{y}=\vec{M} \vec{\theta}$), out of possibly many degenerate
solutions. The OLS objective in Equation~\ref{eq:ols_objective} can be modified
such that it is valid for both the $p<n$ and the $p>n$ case by making use of a
limit in which an L2 regularization term tends to zero
\citep{arXiv:2101.07256}:
\begin{equation}
    \hat{\vec{\theta}}=\lim _{\lambda \rightarrow 0^{+}}\left[
    \underset{\boldsymbol{\theta}}{\operatorname{argmin}}\|\vec{y}-\vec{M} \vec{\theta}\|_{2}^{2} + \lambda \|\vec{\theta}\|_2^2\right]\quad .
\end{equation}
In this case the OLS solution becomes
\begin{equation}
    \hat{\vec{\theta}}=\vec{M}^{\top} (\vec{M} \vec{M}^{\top})^{-1} \vec{y}
...
of $\vec{M}$:
\begin{equation}
    \vec{M}^\dagger\equiv\vec{U} \vec{S}^\dagger \vec{V}^\dagger
\end{equation}
Then $\vec{M}^\dagger\equiv  \vec{V}^{\top} \vec{S}^{\dagger} \vec{U}^{\top}$.

\subsection{Bayesian linear regression}
In the Bayesian paradigm we can write down the posterior over the regression
weights $\vec{\theta}$ assuming a Gaussian likelihood and a Gaussian prior over
the weights. The posterior is then proportional to
\begin{align}
    p(\vec{\theta}|\vec{y}) & \propto p(\vec{y}\lvert \vec{\theta})\,p(\vec{\theta})                                                          \\
                            & =\mathcal{N}(\vec{y}\lvert\vec{M}\vec{\theta},\vec{C})\,\mathcal{N}(\vec{\theta}\lvert \vec{\mu},\vec{\Lambda})
...
The product of two multivariate Gaussian distributions is also a multivariate Gaussian distribution
and the solution to Equation~\ref{eq:bayes_lin_regression} is \citep{arXiv:2005.14199}
\begin{equation}
    \mathcal{N}(\vec{y} \lvert\vec{M} \vec{\theta}, \vec{C}) \,\mathcal{N}(\vec{\theta} \lvert\vec{\mu}, \vec{\Lambda})=\mathcal{N}(\vec{\theta} \lvert\vec{a}, \vec{A}) \,\mathcal{N}(\vec{y} \lvert\vec{b}, \vec{B})
\end{equation}
where
\begin{align}
     & \vec{A}^{-1}   =\vec{\Lambda}^{-1}+\vec{M}^{\top} \vec{C}^{-1} \vec{M}                                                         \\
     & \vec{a}         =\vec{A} \left(\boldsymbol{\Lambda}^{-1} \boldsymbol{\mu}+\vec{M}^{\top} \mathbf{C}^{-1} \boldsymbol{y}\right) \\
     & \mathbf{B}      =\mathbf{C}+\vec{M} \boldsymbol{\Lambda} \vec{M}^{\top}                                                        \\
     & \boldsymbol{b}  =\vec{M} \boldsymbol{\mu}
\end{align}
