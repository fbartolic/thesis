...
%The angular Einstein radius determines a characteristic timescale
%$t_E=\theta_E/|\boldsymbol{\mu}_{LS}|$ on which the magnification of of the
%source changes, where $\boldsymbol{\mu}_{LS}$ is the relative proper motion
%vector between the source and the lens
%$\boldsymbol{\mu}_{LS}\equiv\boldsymbol{\mu}_L- \boldsymbol{\mu}_S$. The
%predicted flux of a star undergoing a microlensing event is then given by the
%following expression
%\begin{equation}
%    f=F_SA(t)+F_B
%    \label{eq:flux_paczynski}
%\end{equation}
%where $F_S$ is the intrinsic flux of the star and
%$F_B$ is any background flux due to other stars in the field which
%might be blended with the source star, usually called the \emph{blending flux}.
%$A(t)$ is a non-linear time-dependant magnification mapping which depends
%on the properties of the lens and the geometry of the lensing event.
%In the simplest case, that of a point source being magnified by a point lens,
%the magnification is given by
%\begin{equation}
%    A(t)= \frac{u(t)^2+2}{u(t)\sqrt{u(t)^2+4}}
%\end{equation}
%where $u(t)$ is the separation between the lens and the source expressed in
...
%\subsection{Binary and triple lens}
...
%corresponding to each point mass $m_i$\footnote{This is because we working
